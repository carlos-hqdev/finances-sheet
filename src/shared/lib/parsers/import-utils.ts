...
export function detectPaymentMethod(description: string): string {
  ...
  if (
    ...
    return "TRANSFER";
  if (
    desc.includes("COMPRA DEB") ||
    desc.includes("DEBITO") ||
    desc.includes("MAQUININHA D") ||
    desc.includes("CARTAO DEB")
  )
    return "DEBIT_CARD";
  if (
    desc.includes("COMPRA CRED") ||
    desc.includes("COMPRA NO CARTAO") ||
    desc.includes("CREDITO")
  )
    return "CREDIT_CARD";
  if (
    desc.includes("SAQUE") ||
    desc.includes("BCO24H") ||
    desc.includes("BANCO24HORAS") ||
