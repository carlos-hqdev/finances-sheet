...
export async function parsePdfFile(
  ...
  const pdf = await Promise.race([
    ...
    new Promise<never>((_, reject) => {
      setTimeout(() => {
        ...
        reject(
          new Error(
            "O processamento do PDF excedeu o limite de tempo. Pode haver um bloqueio de conexão impedindo as bibliotecas auxiliares de carregar.",
          ),
        );
      }, 15000);
    }),
  ...
  if (
    upperText.includes("NU PAGAMENTOS") ||
    upperText.includes("NUBANK") ||
    upperText.includes("NU FINANCEIRA")
  ) {
  ...
  const lineText = groupItemsIntoLines(allItems)
...
function parseNubankPdfItems(
  ...
  for (const line of lines) {
    ...
    const hasTotal =
      upper.includes("TOTAL DE ENTRADAS") || upper.includes("TOTAL DE SA");
    ...
    if (dateMatch && hasTotal) {
      ...
      const valToken = texts.find(
        (t) =>
          signedAmountRegex.test(t) || t.startsWith("+") || t.startsWith("-"),
      );
    ...
    if (pendingDate) {
      ...
      if (valueToken) {
        const descParts = texts.filter(
          (t) =>
            t !== valueToken && !signedAmountRegex.test(t) && t.trim() !== "",
        );
        const description = descParts.join(" ").trim();
        const skipThisLine =
          skipKw.some((k) => upper.includes(k)) || joined.match(/^\d+ de \d+$/);
        ...
        if (!shouldIgnoreTransaction(description) && !skipThisLine) {
          ...
          const finalDescription =
            description || "[A Verificar] Lançamento sem descrição base";
...
function parseC6PdfItems(
  ...
  for (const line of lines) {
    ...
    if (dateMatch && amountMatch) {
      ...
      const amount = parseAmountBR(rawAmountStr);
 ...
 */
function parseBradescoPdfItems(
  items: PdfItem[],
  userDocument?: string,
): ParsedTransaction[] {
  const transactions: ParsedTransaction[] = [];
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    const doctos = texts.filter(
      (t) => doctoRegex.test(t) && !amountRegex.test(t),
    );
    ...
    if (isDataLine) {
      ...
      if (
        !dateInLine &&
        texts.length > 0 &&
        texts.every((t) => !amountRegex.test(t) && !doctoRegex.test(t))
      ) {
 ...
 */
export function parseBrasilPdfText(
  text: string,
  userDocument?: string,
): ParsedTransaction[] {
  const transactions: ParsedTransaction[] = [];
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (dateMatch) {
      ...
      if (finalDate) {
        ...
        const isSummaryLine = summaryKeywords.some((k) =>
          preDateContext.includes(k),
        );
        ...
        const isSameDate =
          lastDate && finalDate.getTime() === lastDate.getTime();
        lastDate = finalDate;
        if (!isSameDate) {
          wordBuffer = [];
        }
    ...
    if (token.includes("R$")) {
      ...
      if (
        !numericValueRegex.test(rawAmountStr) &&
        tokens[i + 1] &&
        numericValueRegex.test(tokens[i + 1])
      ) {
