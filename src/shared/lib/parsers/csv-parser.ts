...
function parseBradescoCSV(
  ...
  const results = Papa.parse<{ [key: string]: string }>(validLines, {
    header: true,
    skipEmptyLines: true,
    delimiter: ";",
  });
  ...
  results.data.forEach((row) => {
    ...
    const desc = rawDesc.trim()
      ? rawDesc.trim()
      : "[A Verificar] Lançamento sem descrição extraída";
...
function parseGenericCSV(
  ...
  const results = Papa.parse<{ [key: string]: string }>(rawContent, {
    header: true,
    skipEmptyLines: true,
  });
  ...
  DATE_COLUMNS.forEach((dc) => {
    if (dateIndex === -1 && normalizedHeaders.includes(dc))
      dateIndex = normalizedHeaders.indexOf(dc);
  });
  AMOUNT_COLUMNS.forEach((ac) => {
    if (amountIndex === -1 && normalizedHeaders.includes(ac))
      amountIndex = normalizedHeaders.indexOf(ac);
  });
  DESC_COLUMNS.forEach((dc) => {
    if (descIndex === -1 && normalizedHeaders.includes(dc))
      descIndex = normalizedHeaders.indexOf(dc);
  });

  if (dateIndex === -1 || amountIndex === -1 || descIndex === -1) {
    throw new Error(
      "Não foi possível identificar as colunas (Data, Valor, Descrição). Formato não suportado por padrão.",
    );
  }
...
export async function parseCSV(
  ...
  if (
    firstLines[0].startsWith("INITIAL_BALANCE") ||
    firstLines.some((l) => l.includes("TRANSACTION_NET_AMOUNT"))
  ) {
