...
import { InvestmentLotTable } from "@/features/investments/components/investment-lot-table";
import { InvestmentDetailActions } from "@/features/investments/components/investment-detail-actions";
...
export default async function InvestmentDetailPage({ params }: InvestmentDetailPageProps) {
  ...
  const [accountsRaw, categories, creditCardsRaw, investmentsListRaw] = await Promise.all([
  ...
  const targetAmount = investment.targetAmount ? investment.targetAmount.toNumber() : null;
  const yieldRate = investment.yieldRate ? investment.yieldRate.toNumber() : null;
  ...
  if (targetAmount && targetAmount > 0) {
    progressPercentage = Math.min(100, Math.max(0, (balance / targetAmount) * 100));
  }
      ...
      <div className="flex-1 space-y-6">
            ...
            <TransactionDialog
              ...
              trigger={
                <Button variant="default" className="gap-2 bg-emerald-600 hover:bg-emerald-700">
                  <ArrowDownCircle className="w-4 h-4" /> {investment.type === "FIXED" ? "Guardar" : "Aportar"}
                </Button>
              }
            />
            <TransactionDialog
              ...
              trigger={
                <Button variant="outline" className="gap-2 text-rose-500 hover:text-rose-600 border-rose-500/20 hover:bg-rose-500/10">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          {investment.type === "FIXED" && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium">Rendimento Configurado</CardTitle>
                <Percent className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {yieldRate
                    ? `${yieldRate}% ${investment.indexer
                      ? (investment.indexer === "PREFIXED" ? "Prefixado" : investment.indexer === "OTHER" ? "" : investment.indexer)
                      : (investment.isDailyYield ? "CDI" : "")
