...
function parseBrasilPdfText(text) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    } else if (text.toUpperCase().includes('BRADESCO') && numericValueRegex.test(token)) {
      amount = parseAmountBR(token);
    }

    if (amount !== null && lastDate) {
      let description = wordBuffer.join(' ') || null;
      if (!description && lastDate) description = `Movimentação (PDF)`;

      if (!shouldIgnoreTransaction(description)) {
        // Detecção de sinal simplificada para teste
        if (text.toUpperCase().includes('BRADESCO') && !token.includes('-')) {
      ...
      // PERSISTÊNCIA DA DATA (NOVA LÓGICA)
      wordBuffer = [];
      continue;
    }
