...
function parseBradescoPdf(items) {
  ...
  for (const line of lines) {
    ...
    if (isDataLine) {
      ...
      const txVals = numVals.slice(0, -1);
...
async function getPdfTransactions(pdfPath) {
  ...
  return parseBradescoPdf(allItems);
}

function getCsvCount(csvPath) {
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    const credit = parseAmountBR(row[3]);
    const debit = parseAmountBR(row[4]);
...
const dir = '/home/carlo/Projetos/Pessoal/finances-sheet/extratos_exemplos/Bradesco';

for (const year of years) {
  const csvCount = getCsvCount(`${dir}/${year}.csv`);
  const pdfTxs = await getPdfTransactions(`${dir}/${year}.pdf`);
  const match = csvCount === pdfTxs.length ? '✓ OK' : '✗ MISMATCH';
  console.log(`${year}: CSV=${csvCount} | PDF=${pdfTxs.length} | ${match}`);
}
